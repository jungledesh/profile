...
| **R2b KV admission backlog**  | Same 88% near-full bar, queue ratio at or above 30%, running below `--max-num-seqs`, free KV tokens below waiting requests × mean prompt tokens. Mutually exclusive with R2 via eval `else if`, not the suppression table. | Same family, without the eviction signal                                                                                                                                                                   |
