...
- **The mean is one collection.** A diagnose run aggregates many 2s or 10s slices into one summary. Cyclic load averaged over that duration can look like a state it is not. Raise `--duration` so several cycles fit, up to `30m`. Load that changes between iterations is not a duration problem. Shapes: [workflow.md](workflow.md#duration-and-traffic-shape).
