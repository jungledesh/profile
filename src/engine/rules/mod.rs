...
fn compute_kv_max_seqs_with_mode<const MULTI_GPU: bool>(
    ...
    // Resharding heads needs a mutated copy; the tp=1 launch path prices the
    // borrowed arch directly, no clone.
    let request_bytes = if tp > 1 {
        ...
        bytes_per_seq(&priced_model, max_len, kv_bpp)
    } else {
        bytes_per_seq(model, max_len, kv_bpp)
    };
    let Some(request_bytes) = request_bytes else {
        return DerivedCapacity::default();
    };
    let observed = cache.and_then(|c| observed_budget_bytes(c, model, kv_bpp));
