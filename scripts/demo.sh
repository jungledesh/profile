#!/usr/bin/env bash
# Demo setup — Qwen2.5-Coder-7B-Instruct on H100
# Coding assistant scenario: shared system prompt, prefix caching on, low max_num_seqs to trigger R5.

set -Eeuo pipefail
trap 'echo "FAILED at line $LINENO"' ERR

# Pinned versions — kept in sync with start.sh
PIP_VERSION="${PIP_VERSION:-26.0.1}"
UV_VERSION="${UV_VERSION:-0.11.1}"
VLLM_VERSION="${VLLM_VERSION:-0.18.0}"
HUGGINGFACE_HUB_VERSION="${HUGGINGFACE_HUB_VERSION:-0.36.2}"
TORCH_BACKEND="${TORCH_BACKEND:-cu126}"

APP_DIR="${APP_DIR:-/home/appuser/app}"
VENV_DIR="${VENV_DIR:-/home/appuser/vllm-env}"
MODELS_DIR="${MODELS_DIR:-/workspace/models}"
MODEL_PATH="${MODEL_PATH:-$MODELS_DIR/qwen25-coder-7b}"
TMUX_SESSION="${TMUX_SESSION:-vllm}"
LOG_FILE="${APP_DIR}/vllm.log"

echo "Starting demo environment..."

mkdir -p "$APP_DIR" "$MODELS_DIR"

# Venv
if [[ ! -f "$VENV_DIR/bin/activate" ]]; then
    rm -rf "$VENV_DIR"
    python3 -m venv "$VENV_DIR"
fi
source "$VENV_DIR/bin/activate"

python -m pip install "pip==${PIP_VERSION}"
python -m pip install "uv==${UV_VERSION}"
uv pip install "vllm==${VLLM_VERSION}" --torch-backend="${TORCH_BACKEND}"
uv pip install "huggingface-hub==${HUGGINGFACE_HUB_VERSION}"

if [[ -n "${HF_TOKEN:-}" ]]; then
    export HF_TOKEN
fi

HF_CLI="${VENV_DIR}/bin/huggingface-cli"

# Download model if not present
if [[ ! -d "$MODEL_PATH" ]] || [[ -z "$(ls -A "$MODEL_PATH" 2>/dev/null)" ]]; then
    echo "Downloading Qwen2.5-Coder-7B-Instruct..."
    mkdir -p "$MODEL_PATH"
    [[ -x "$HF_CLI" ]] || { echo "missing $HF_CLI after hub install" >&2; exit 1; }
    "$HF_CLI" download \
        Qwen/Qwen2.5-Coder-7B-Instruct \
        --local-dir "$MODEL_PATH"
else
    echo "Model already present."
fi

# Kill existing session
if tmux has-session -t "$TMUX_SESSION" 2>/dev/null; then
    echo "Killing existing tmux session: $TMUX_SESSION"
    tmux kill-session -t "$TMUX_SESSION"
fi

# Demo config:
#   --enable-prefix-caching     R3 path A fires if hit rate is low; flip off to test path B
#   --max-num-seqs 32           intentionally low — R5 fires under demo load
#   --gpu-memory-utilization 0.85  more KV headroom for the coder workload
#   --max-model-len 8192        coder context is longer
tmux new-session -d -s "$TMUX_SESSION" \
"bash -lc 'source \"$VENV_DIR/bin/activate\" && \
python -m vllm.entrypoints.openai.api_server \
  --model \"$MODEL_PATH\" \
  --served-model-name Qwen2.5-Coder-7B-Instruct \
  --host 0.0.0.0 \
  --port 8000 \
  --dtype auto \
  --gpu-memory-utilization 0.85 \
  --tensor-parallel-size 1 \
  --enforce-eager \
  --max-model-len 8192 \
  --max-num-seqs 32 \
  --enable-prefix-caching \
  2>&1 | tee \"$LOG_FILE\"'"

echo
echo "vLLM running in tmux session '$TMUX_SESSION'"
echo "Model: Qwen2.5-Coder-7B-Instruct"
echo "Attach: tmux attach -t $TMUX_SESSION"
echo ""
echo "Run profile:"
echo "  profile diagnose --url http://localhost:8000/metrics --duration 2m -m 32"

if [[ -t 0 ]]; then
  exec bash -l
else
  tail -f /dev/null
fi
